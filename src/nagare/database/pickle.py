...
def add_pickle_hooks(mapper, cls):
    ...
    if not hasattr(cls, '__getstate__') or (cls.__getstate__ is getattr(object, '__getstate__', None)):
        cls.__getstate__ = entity_getstate

    if not hasattr(cls, '__setstate__') or (cls.__setstate__ is getattr(object, '__setstate__', None)):
